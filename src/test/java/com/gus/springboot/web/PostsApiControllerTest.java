...
// @WebMvcTest 경우 JPA 기능이 작동하지 않음.
// JPA는 애플리케이션과 JDBC API 사이에서 동작한다.
// JPA를 사용하려면, @SpringBootTest 와 TestRestTemplate 을 사용한다.
public class PostsApiControllerTest {
    ...
    @Test
    @WithMockUser(roles="USER") // 테스할 때, 가짜사용자를 만들어 roles 에 권한을 추가함
...
//        ResponseEntity<Long> responseEntity = restTemplate.postForEntity(url, requestDto, Long.class);
        mvc.perform(post(url) // 생성된 MockMvc 통해 API 를 테스트
                .contentType(MediaType.APPLICATION_JSON_UTF8)
                // Body 영역은 문자열로 표현하기 위해 ObjectMapper 통해 문자열 JSON 으로 변환
